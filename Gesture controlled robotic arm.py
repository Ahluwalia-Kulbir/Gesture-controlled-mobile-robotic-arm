...
ser = serial.Serial('COM8', 9600) #for Xbee at COM8
#Use "ser = serial.Serial('COM8', 9600,timeout=None)" for a wired USB connection with the Arduino
...
def createMasks(array, hueMod, satValMod,img):
    ...
    for i in range(0,len(array)):
        ...
        mask = cv2.inRange(img, lower,upper) #variable colour mask
...
def enclosingCircle(contour,img):
    (x,y),radius = cv2.minEnclosingCircle(contour) #tuple of coordinates
    center = (int(x),int(y))
    radius = int(radius)
...
print(3)
cv2.setMouseCallback('setup',setRectPoints) #to allow recording/input from mouse
print(4)
...
while(1):
    ...
    cv2.circle(circled,center,5,(0,0,255),2) #thickness, colour, alpha 
    cv2.imshow('running',circled)
    ...
    print("Circle radius=" + str(radius) + "     Circle centre coordinates=" + str(center))
    
    move(1, servoLR)
    move(2, servoFB)
    move(3, servoTB)
    move(4, claw)
    k  = cv2.waitKey(1) & 0xFF #Waits for escape key, ASCII value of escape =27
    if k == 27:
        break
    elif k == ord('-'):
        claw = min(claw + 5,90)
       #print(str(claw))
        ...
        claw = max(0,claw - 5)
        #print(str(claw))
    print("Left-Right="+str(servoLR)+ "   Front-Back="+str(servoFB)+"   Top-Bottom="+str(servoTB)+"   Claw="+str(claw))
    print("")
